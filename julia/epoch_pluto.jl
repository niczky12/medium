...
# ╔═╡ 3897a6ed-9d94-40ac-81f4-519350337787
import Pkg

# ╔═╡ e4460c30-1793-11ec-1a97-158fcd41fe50
using Flux

# ╔═╡ fb4e70b1-dc84-4329-ae98-6c875e9f7ccb
using PlutoUI

# ╔═╡ 567430c4-eff6-41c1-8fb9-07eb5289ceb3
using Plots

# ╔═╡ 0474074f-3c2a-4689-b848-633ed4ab526a
using IterTools

# ╔═╡ acdaea74-9c3c-4b9f-8026-4b9553d057bd
using Dates

# ╔═╡ 5a3b550a-575a-4913-980c-71a7322c1428
md"""
# Your first useless Neural Network with Flux.jl

In this tutorial, I'll show you how to predict unix epochs (seconds since 1st January 1970) from a datetime.
Yes, you should not do this, but this is fun and allows you play around with Neural Nets in Flux.jl.

This notebook is also published as a medium article. So check out [my medium page](https://medium.com/@niczky12).
"""

# ╔═╡ a618ae4d-dd3d-416f-8c20-522291ae3730
import Random

# ╔═╡ 66d0c0b1-4de9-4f3b-a093-b41faad67ba1
gr()

# ╔═╡ 74ca7ba4-2460-4eb1-bd68-91230a0af8ff
begin
...
# ╔═╡ cc5017ab-da33-4263-8c11-d6fd8a493a12
md"""
## Normalising inputs and outputs

Since our inputs will range from 2010 to 2030 and out outputs will be on the scale of 1e9 we need to normalise these so that we can do without huge weights.
This actually took me some time to figure out.
"""

# ╔═╡ 8b3af7f7-b9d2-4a86-a16f-9471c9651031
begin
...
# ╔═╡ adc3c01c-68ae-4816-a225-d31d01db458d
# get some values that we need to predict
# feel free to add more values, or change the seed
begin
...
# ╔═╡ 06a2f423-33fc-4525-9058-71e2a4a7ccd4
# change this if you want to play around with minibatch sizes
batch_size = 10

# ╔═╡ 5e781aa3-d7d5-4c5e-a418-9de5e6ac443e
xs = hcat.(partition(datetime2input.(unix2datetime.(y)), batch_size)...)

# ╔═╡ eb848b18-2286-443a-93e0-5845d060ee17
ys = hcat.(partition(y_normaliser.(y), batch_size)...)

# ╔═╡ f3acb65b-8333-4820-8511-c3918b80e7f8
begin
...
# ╔═╡ 5828164d-c6ca-41d4-916e-e936d6c7407b
md"""
## Time to build a network

You can use more complex multi-layer networks if you want to:
...
# ╔═╡ d136fd63-4c6c-49b9-a1df-2736279efb31
model = Dense(6, 1, identity)

# ╔═╡ 3f084842-4cb3-4ea0-a71b-3ece341d1661
# try different loss functions, I use mean squared error here
loss(x, y) = Flux.mse(model(x), y)

# ╔═╡ 0308af11-d176-43a1-863a-68577f750f26
# you can use different optimisers too!
optimizer = ADAM()

# ╔═╡ 702de493-a5e6-4ce4-80e6-32806f4db4a5
# I recommend you to change the number of epochs to something lower if you want to
...
# ╔═╡ 93a9ed05-80d8-4219-945f-26a27753311a
plot(epochs, training_loss; title="Training loss 2010-2020")

# ╔═╡ ccfb0441-e99d-4569-a4b2-80531ddc0ebb
plot(epochs, testing_loss; title="Testing loss 2020-2030")

# ╔═╡ b6693116-7394-4f84-a777-52664e816dcc
md"""
## Look at some examples
"""

# ╔═╡ 01194f7f-074a-49fa-9af5-4e4a60152573
function predict_epoch(dt)
	# we need the [1] there because the model predicts an array
    y_rescaler(model(datetime2input(dt))[1])
end

# ╔═╡ b31577fc-f2fc-4dbd-a1e5-c7eeba50f347

with_terminal() do
	
println("Input -> Predicted epochs as datetime | difference")
...
# ╔═╡ 277f58cf-80a8-424c-82fd-70a6d6efa6e3
scatter(
	unix2datetime.(y_test),
	round.(
		unix2datetime.(y_test) .- unix2datetime.(predict_epoch.(unix2datetime.(y_test))), Hour);
	title="Prediction errors outside the training cases.")

# ╔═╡ 272329c6-3612-4d04-a04c-0796bb50fb3a
md"""
## Notes for reader

- You can use callbacks in Flux instead of printing every n epochs.
- Try replacing the original input and output arrays with CUDA arrays - if you have a compatible graphics card.
- Try new architectures, activation functions, optimisers
- It looks like there are some patterns in the errors. Can you add specific examples to correct for this?
- Can you add more examples, would the error go down?
- Are the errors getting worse with time?
- How about predicting between 2010 and 2020?
"""
